...
def tract_3D(bundle_filename="AL26_clean.tck",
    ...
    if colors == "fa":
        ...
        colors = fa
    stream_actor = actor.line(streamlines, colors, linewidth=1,
    ...
    x_midpoint = int(np.round(shape[0] / 2))
    image_actor_x.display_extent(x_cut, x_cut,
                                0, shape[1] - 1,
                                0, shape[2] - 1)
    ...
    z_slicer_opacity = 1
    x_slicer_opacity = 1
    y_slicer_opacity = 0.75
    image_actor_z.opacity(z_slicer_opacity)
    
    image_actor_x.opacity(x_slicer_opacity)
    image_actor_y.opacity(y_slicer_opacity)
    """
    ...
    opacity_slider = ui.LineSlider2D(min_value=0.0,
                                    max_value=1.0,
                                    initial_value=y_slicer_opacity,
                                    length=140)
    ...
    scene.set_camera(camera[0], camera[1], camera[2])
    ...
    if interactive:
        ...
        show_m.render()
        show_m.start()
        x_cut = image_actor_x.GetDisplayExtent()[0]
        y_cut = image_actor_y.GetDisplayExtent()[2]
        z_cut = image_actor_z.GetDisplayExtent()[4]
    ...
    print(scene.get_camera())
    return {"x_cut":x_cut, "y_cut":y_cut, "z_cut":z_cut,
            "camera": camera}
